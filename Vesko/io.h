...
IOMOD * IoNewMod();
DLLENTRY(BOOL) IoGetStat(byte addr);
DLLENTRY(BOOL) IoInBitVal(byte addr, int bitnum);
DLLENTRY(BOOL) IoInBitSVal(byte addr, int bitnum);
DLLENTRY(BOOL) IoOutBitVal(byte addr, int bitnum);
DLLENTRY(byte) IoGetADCVal(byte addr, int channel);
DLLENTRY(byte) IoGetPWMVal(byte addr, int channel);
DLLENTRY(unsigned long) IoGetTimerVal(byte addr);
DLLENTRY(unsigned long) IoGetTimerSVal(byte addr);
DLLENTRY(unsigned short int) IoGetOutputs(byte addr);
DLLENTRY(byte) IoGetTimerMode(byte addr);
DLLENTRY(BOOL) IoSetOutputs(byte addr, unsigned short int outval);
DLLENTRY(BOOL) IoSetOutBit(byte addr, int bitnum);
DLLENTRY(BOOL) IoClrOutBit(byte addr, int bitnum);
DLLENTRY(BOOL) IoSetPWMVal(byte addr, byte pwm1, byte pwm2);
DLLENTRY(BOOL) IoSetTimerMode(byte addr, byte tmrmode);
DLLENTRY(BOOL) IoSetSynchOutput(byte addr, short int outbits, byte pwm1, byte pwm2);
DLLENTRY(BOOL) IOSynchOutput(byte groupaddr);
DLLENTRY(BOOL) IoGetBitDir(byte addr, int bitnum);
DLLENTRY(BOOL) IoBitDirOut(byte addr, int bitnum);
DLLENTRY(BOOL) IoBitDirIn(byte addr, int bitnum);
